...
function BlogCard({
  ...
  return (
    <div className="w-full flex justify-center">
      ...
      <div className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] 
      border border-[#E8EEF4] overflow-hidden w-[95%] max-w-5xl my-8 flex flex-col md:flex-row">

        <div className="relative md:w-2/5 w-full h-60 md:h-auto">
          <img
            src={categoryImage}
            alt={category}
            className="w-full h-full object-cover"
          ...
          <button
            onClick={toggleHeart}
            className="absolute top-3 left-3 bg-white shadow-md p-2 rounded-full hover:scale-110 transition"
          >
            {liked ? (
        ...
        <div className="p-6 md:w-1/2 flex flex-col justify-between">
          ...
          <div className="flex items-center gap-6 text-gray-500 text-sm mb-4">
            ...
            >
              {thumbLiked ? (
                <FaThumbsUp className="text-[#00A9FF]" />
              ) : (
                <FaRegThumbsUp />
              )}
              {thumbCount}
          ...
          {status === "published" ? (
            <Link
              to={`/blog/${slug}`}
              className="inline-block bg-gradient-to-r from-[#00A9FF] to-[#89CFF3]
              text-white px-5 py-2 rounded-lg shadow-md hover:scale-[1.03] 
              hover:shadow-lg transition font-medium w-fit"
            >
              Read More
            </Link>
          ) : (
            <Link
              ...
              shadow-md hover:bg-gray-800 transition font-medium w-fit"
            >
              Edit Blog
            </Link>
          )}
